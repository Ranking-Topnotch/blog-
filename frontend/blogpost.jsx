...
const Newblog = () => {
    ...
    const uploadImage = async (type) => {
        ...
        try{
            ...
            let resourceType = type === 'image' ? img : 'video';
  ...
  return (
    <div className={style.postHead}>
        <h3>What do you have for us?</h3>
        <form className={style.form} onSubmit={handlePostSubmit}> 
            <input id='image' className={style.imageUpload} type={'file'} placeholder="image" accept="image/*" onChange={(e) => setImg((prev) => e.target.files[0])} />
            <button type={'submit'}>Post</button>
        </form>
    </div>
  )
